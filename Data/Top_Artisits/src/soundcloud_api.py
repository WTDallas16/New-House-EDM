...
class SoundCloudAPIClient:
    ...
    def resolve(self, url: str) -> dict[str, Any]:
        payload = self.get_json("/resolve", url=url)
        return payload if isinstance(payload, dict) else {}

    def search_users(self, query: str, limit: int = 5) -> list[dict[str, Any]]:
        payload = self.get_json("/users", q=query, limit=limit)
        if isinstance(payload, list):
            return [item for item in payload if isinstance(item, dict)]
        collection = payload.get("collection") if isinstance(payload, dict) else None
        return [item for item in collection or [] if isinstance(item, dict)]

    def user_tracks(self, user_id: str, limit: int = 50) -> list[dict[str, Any]]:
        payload = self.get_json(f"/users/{user_id}/tracks", limit=limit, linked_partitioning=1)
        if isinstance(payload, list):
            return [item for item in payload if isinstance(item, dict)]
        collection = payload.get("collection") if isinstance(payload, dict) else None
        return [item for item in collection or [] if isinstance(item, dict)]
