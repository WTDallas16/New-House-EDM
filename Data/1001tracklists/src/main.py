...
from src.scrapers.tracklists1001 import Tracklists1001AccessError, Tracklists1001Scraper

LOGGER = logging.getLogger(__name__)

SCRAPERS: dict[str, type[BaseScraper]] = {
    "1001tracklists": Tracklists1001Scraper,
}
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--html-input", default=None, help="Parse a saved HTML page instead of fetching the URL.")
    parser.add_argument(
        "--cookie-header",
        default=None,
        help="Cookie header from a browser session. Prefer TRACKLISTS1001_COOKIE to avoid shell history.",
    )
    parser.add_argument(
        "--chart-fallback",
        action=argparse.BooleanOptionalAction,
        default=True,
        help="If the genre page is challenged, fall back to unblocked 1001Tracklists chart pages.",
    )
...
def extract_releases(
    ...
    if source == "1001tracklists":
        scraper = scraper_cls(html_input=html_input, cookie_header=cookie_header, chart_fallback=chart_fallback)
    else:
        scraper = scraper_cls()
    if hasattr(scraper, "scrape_releases"):
        direct_candidates = scraper.scrape_releases(url, lookback_days=lookback_days, max_pages=max_pages)
        return dedupe_releases(direct_candidates)
...
def main() -> None:
    ...
    try:
        releases = extract_releases(
            ...
            chart_fallback=args.chart_fallback,
        )
    except Tracklists1001AccessError as exc:
