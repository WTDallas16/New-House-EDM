...
from weraveyou.src.extraction.classifier import classify_article
from weraveyou.src.extraction.normalize import clean_text
from weraveyou.src.models import ArticleCard, ArticleEnrichment, ReleaseCandidate

LOGGER = logging.getLogger(__name__)

QUOTE_CHARS = "'\"‘’“”"
QUOTE_PATTERN = r"[\"'‘“](?P<title>[^\"'‘’“”]{2,160})[\"'’”]"

TITLE_PATTERNS: list[tuple[str, str]] = [
    (
        "regex",
        rf"^(?P<artist>.+?)\s+(?:releases?|drops?|unveils?|delivers?|shares?|returns with|channels|continues .*? with|enlists? .+? for|details .*? and shares?|announces .*? and unveils?|announces .*?,\s*unveils?|joins? forces .*? (?:for|on|with)|teams up .*? (?:for|on|with)|collaborates .*? (?:for|on|with)).*?{QUOTE_PATTERN}",
    ),
    (
        "regex",
...
TRAILING_NOISE_RE = re.compile(
    r"\s*(?::\s*Listen|\|\s*.*$|\s+-\s+We Rave You.*$|\s+\[[^\]]+\])\s*$",
    re.IGNORECASE,
)
...
def extract_from_title(title: str) -> tuple[str, str, str] | None:
    ...
    for method, pattern in TITLE_PATTERNS:
        ...
        extracted_title = _clean_extracted_title(match.group("title"))
...
def parse_release_candidate(
    ...
    if parsed:
        ...
        if enrichment:
            ...
            extraction_method = "article_metadata" if artist else "article_body"
        if not project_title:
            LOGGER.info("Could not extract release title from %r", article.title)
            return None
        if not artist:
            artist = clean_text(article.title.split(" ", 1)[0])
