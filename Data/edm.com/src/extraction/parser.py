...
TITLE_PATTERNS: list[tuple[str, str]] = [
    (
        "regex",
        rf"^.*?Listen to (?P<artist>.+?)['’]s .+?\b(?:single|track|banger|anthem|release|remix|rework|EP|album).*?{QUOTE_PATTERN}",
    ),
    (
        "regex",
        r"^(?P<artist>.+?)['’]s\s+[‘“'\"](?P<title>[^’”'\"]{2,160})[’”'\"]\s+(?:album\b|.+?\balbum\b)",
    ),
    (
        "regex",
        rf"^(?P<artist>.+?)\s+to\s+.+?new\s+single.+?listen\s+to\s+{QUOTE_PATTERN}",
    ),
    (
        "regex",
        rf"^.+?\s+on\s+(?P<artist>.+?)['’]s\s+{QUOTE_PATTERN}",
    ),
    (
        "regex",
        rf"^.+?\s+in\s+(?P<artist>.+?)['’]s\s+.+?{QUOTE_PATTERN}",
    ),
    (
        "regex",
        rf"^(?P<artist>.+?)\s+(?:announces?.*?with release of|releases?|drops?|unveils?|reveals?|unleash(?:es)?|delivers?|presents?|shares?|returns? with|returns? .*? with|extends .*? with|starts? .*? with|goes .*? with|gets .*? with|channels|continues .*? with|enlists? .+? for|recruits? .+? for|details .*? and shares?|announces .*? and unveils?|announces .*?,\s*unveils?|joins? .*? with|joins? forces .*? (?:for|on|with)|teams up .*? (?:for|on|with)|collaborates .*? (?:for|on|with)|collides?\s+(?:for|on|with)|collides? .*? (?:for|on|with)|unites?\s+on|reunites?\s+for|lands? .*? with|provides? .*? in|pulls? .*? on|brings? .*? on).+?{QUOTE_PATTERN}",
    ),
    (
        "regex",
    ...
    (
        "regex",
        r"^(?P<artist>.+?)\s+.+?\b(?:EP|album|single|track),?\s+[‘“'\"](?P<title>[^’”'\"]{2,160})$",
    ),
    (
        "regex",
...
def clean_article_title(title: str) -> str:
    title = re.sub(r"^Exclusive:\s+", "", title, flags=re.IGNORECASE)
