...
def classify_article(title: str, snippet: str | None = None) -> ClassificationResult:
    ...
    if "fresh picks" in title_norm or "on-deck circle" in title_norm:
        return ClassificationResult(
            release_article=False,
            confidence_score=0.0,
            include_signals=include_signals,
            exclusion_signals=list(dict.fromkeys(exclusion_signals + ["roundup"])),
            clearly_dropped_release=clearly_dropped,
        )
    if "next single" in title_norm and ": listen" not in title_norm:
        return ClassificationResult(
            release_article=False,
            confidence_score=0.0,
            include_signals=include_signals,
            exclusion_signals=list(dict.fromkeys(exclusion_signals + ["next single"])),
            clearly_dropped_release=clearly_dropped,
        )
